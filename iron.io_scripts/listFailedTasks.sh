...
## get failed tasks for the last week
if [ -z $NUM_DAYS ]; then
  ## thismay be a lot
  NUM_DAYS=7
fi
...
TOTAL_TASKS=0
# Iterate thru these 1 day at a timeout
...
do
  ...
  getTasksForOneDayAProject ${core_v3_prod} core_v3_prod ${outdir} $FROM_DAY $QFILTER
  TOTAL_TASKS=$((TOTAL_TASKS + TOTAL_DAY_TASKS))
done
