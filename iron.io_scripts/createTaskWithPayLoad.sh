...
function getRidOfSpaces() {
    #echo "ORIGINAL=$1"
    ret_val=$(sed 's/ //g' <<< "$1")
    #echo "MODIFIED=${ret_val}"
}
...
function createTaskWithAPayload() {
  proj=$1
  projName=$2
  payload=$3
  code_name=$4
  ...
  echo "=================== ${projName} ======================="
  echo "Creating job for $code_name"
  ...
  echo ------------------------
      ...
      -d '{"tasks": [{"code_name": "'$code_name'","cluster":"'$cluster'","payload": '$payload'}]}'
      set +x
}
...
CODE_NAME=transcode-ffmpeg
CLUSTER=58f79d2d35eec2000a21bb6a
if [ -z $PAYLOAD_FILE ];
then
...
tmp1=$PAYLOAD_FILE
            ...
            (( ntasks-- ))
            for i in $(seq 0 $ntasks); do
                recording_id=`cat ${tmp1} | jq ".tasks[${i}].recording_id"`
                payload=`cat ${tmp1} | jq ".tasks[${i}].payload"`
                #echo $payload
                getRidOfSpaces "$payload"
                payload=${ret_val}

                echo -------------------------
                echo Task[$i]: $recording_id
                createTaskWithAPayload ${core_v3_prod} core_v3_prod "$payload" $CODE_NAME $CLUSTER

                #may want to throttle after every 50 or something by sleeping 30seconds
                n=$(( i%$interval ))
                if (( n==0 )); then
                   echo "$i DINGDINGDING sleeping for 20s......................................."
                   sleep 20s
                fi
            done
